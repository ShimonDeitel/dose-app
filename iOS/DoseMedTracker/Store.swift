...
final class Store: ObservableObject {
    ...
    private let fileName = "dose_entries.json"
    ...
    static func seedData() -> [Entry] {
        let now = Date()
        return [
            Entry(title: "Sample Medication One", detail: "First example entry.", date: now),
            Entry(title: "Sample Medication Two", detail: "Second example entry.", date: now.addingTimeInterval(-86400)),
            Entry(title: "Sample Medication Three", detail: "Third example entry.", date: now.addingTimeInterval(-172800)),
        ]
    }
}
